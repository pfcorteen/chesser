...
export class ComputedMove {
     ...
     squareValueReOccupy = ([mpid, sqid]: PID_TO): number => {
          ...
          while (shifted = attckrMove ? attckrsWithRank.shift() : dfndrsWithRank.shift()) {
               const
                    [pid, rank] = shifted,
                    oppExchanger = attckrMove
                         ? (dfndrsWithRank.length ? true : false)
                         : (attckrsWithRank.length ? true : false);
